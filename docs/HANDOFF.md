...
Everything should be green: **478 backend tests, 150 frontend tests**, coverage 96%+
...
How a weapon is held is an axis of the attack lines, not one extra row: a one-handed
weapon offers both grips and every feat variant is generated for each, since
two-handed *with* Power Attack is the line a player reaches for. A two-handed line
carries its own AC, because the shield it cannot hold is not helping that round — the
buckler excepted, which stays on for -1 to attack.
...
Levelling up is a report, not a wizard: `POST /level-up-preview` says what one more
level in a named class buys — the class' hit die and each save before → after, the
feat or ability increment owed to the character's *total* level, the class' own
`especial` — and applies none of it. Growing past a level files a full copy of the
document, keyed by the level it was, so a past level opens in the same combat card as
the present one. The copy is a record, not an editable timeline.
...
1. **Two-weapon fighting is still unreachable.** `_twf` looks for an `off_hand`
   weapon and the editor has no way to make one, so the whole TWF path — iteratives,
   the -2/-4/-6 penalties, `Combate con dos armas` — is dead in the app while fully
   modelled and tested in the domain. The duplicate-weapon guard (2026-08-13) exists
   *because* of this and should be lifted with it: two short swords, one per hand, is
   a real build. Grip itself is done — one-handed weapons offer both, two-handed ones
   are forced correctly — so what is missing is naming the off hand.
