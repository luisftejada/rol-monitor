...
/** The `.attack` block whose plain (un-suffixed) name is exactly `name`. */
function attackBlock(name: string): HTMLElement {
  const label = screen.getByText(name, { selector: "p" });
  const block = label.closest(".attack");
  if (!(block instanceof HTMLElement)) throw new Error(`no .attack block for "${name}"`);
  return block;
}
...
describe("AttacksSection", () => {
  ...
  it("shows each equipped weapon's attack and damage lines", () => {
    ...
    expect(screen.getByText(/Crítico: 19-20\/×2/)).toBeInTheDocument();
  });
  ...
  // toggle — the frontend renders whatever it is given, one block per entry.
  it("lists each way of using a weapon as its own line", () => {
    ...
    // Both lines start with "Espada larga", so each is scoped to its own block
    // (found by its exact, un-suffixed name) before reading its attack bonus.
    const plain = attackBlock("Espada larga");
    expect(within(plain).getByRole("button", { name: /^Espada larga/ })).toHaveTextContent("+4");

    const powered = attackBlock("Espada larga (Ataque poderoso)");
    expect(within(powered).getByRole("button", { name: /^Espada larga/ })).toHaveTextContent("+2");
  ...
  it("expands an attack line to show its breakdown", async () => {
    ...
    renderWithProviders(<AttacksSection attacks={fighterSheet.attacks} />);
    await user.click(screen.getByRole("button", { name: /Espada larga/ }));
    expect(screen.getByRole("region", { name: /Espada larga/ })).toHaveTextContent("Fue");
  });
});
